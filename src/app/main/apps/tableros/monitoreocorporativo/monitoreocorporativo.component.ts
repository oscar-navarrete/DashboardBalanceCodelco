...
@Component({
  selector: 'app-monitoreocorporativo',
  templateUrl: './monitoreocorporativo.component.html',
  styleUrls: ['./monitoreocorporativo.component.scss']
})
export class MonitoreocorporativoComponent implements OnInit {
  ...
  async getMonitoreo() {
    ...
    await this.ds.getDataFromAPI(body).then((data: any) => {
      ...
      // console.log(this.valores[0].value);
  ...
  graficoDch(data: any): void {
    const info: any = this.getGaugueConfig(data, 1, `DIVISION`, ' CHUQUICAMATA', '%');
    setTimeout(() => {
      this.DCH = this.objects.getGaugue(info);
      this.updateFlag = true;
    }, 2000);
  }

  graficoDrt(data: any): void {
    const info: any = this.getGaugueConfig(data, 2, `DIVISION`, ' RADOMIRO TOMIC', '%');
    setTimeout(() => {
      this.DRT = this.objects.getGaugue(info);
      this.updateFlag = true;
    }, 2000);
  }

  graficoDmh(data: any): void {
    const info: any = this.getGaugueConfig(data, 3, `DIVISION`, ' MINISTRO HALES', '%');
    setTimeout(() => {
      this.DMH = this.objects.getGaugue(info);
      this.updateFlag = true;
    }, 2000);
  }

  graficoDgm(data: any): void {
    const info: any = this.getGaugueConfig(data, 4, `DIVISION`, ' GABRIELA MISTRAL', '%');
    setTimeout(() => {
      this.DGM = this.objects.getGaugue(info);
      this.updateFlag = true;
    }, 2000);
  }

  graficoDsa(data: any): void {
    const info: any = this.getGaugueConfig(data, 5, `DIVISION`, ' EL SALVADOR', '%');
    setTimeout(() => {
      this.DSA = this.objects.getGaugue(info);
      this.updateFlag = true;
    }, 2000);
  }

  graficoDve(data: any): void {
    const info: any = this.getGaugueConfig(data, 6, `DIVISION`, ' VENTANAS', '%');
    setTimeout(() => {
      this.DVE = this.objects.getGaugue(info);
      this.updateFlag = true;
    }, 2000);
  }

  graficoDan(data: any): void {
    const info: any = this.getGaugueConfig(data, 7, `DIVISION`, ' ANDINA', '%');
    setTimeout(() => {
      this.DAN = this.objects.getGaugue(info);
      this.updateFlag = true;
    }, 2000);
  }

  graficoDte(data: any): void {
    const info: any = this.getGaugueConfig(data, 8, `DIVISION`, ' EL TENIENTE', '%');
    setTimeout(() => {
      this.DTE = this.objects.getGaugue(info);
      this.updateFlag = true;
    }, 2000);
  }

  getGaugueConfig(data: any, id: number, title: string = '', subtitle: string = '', um: string = ''): any {
