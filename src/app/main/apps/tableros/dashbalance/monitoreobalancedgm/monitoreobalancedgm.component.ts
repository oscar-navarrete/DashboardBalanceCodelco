...
@Component({
  selector: 'app-monitoreobalancedgm',
  templateUrl: './monitoreobalancedgm.component.html',
  styleUrls: ['./monitoreobalancedgm.component.scss']
})

export class MonitoreobalancedgmComponent implements OnInit {
  ...
  MonitoreoDMH(data: any): void {
    const infoanterior: any = this.getGaugueConfig(data[3], data[3].Calidad_Dato_Mes_Anterior, 1, `MES`, ' ANTERIOR', '');
    const infoactual: any = this.getGaugueConfig(data[3], data[3].Calidad_Dato_Mes_Actual, 2, `ACUMULADO`, ' MES ACTUAL', '');
    setTimeout(() => {
  ...
  async getDatosAreas() {
    const body: any = this._service.getBody('DGM', 'ds-001');
    await this.ds.getDataFromAPI(body).then((data: any) => {
      const myData: any = data;

      const elementosDgm: any = myData['ElementosAreaDGM'].Content.Items;
      const AtributosDgm: any = myData['AttributosAreaDGM'].Content.Items;
      const ValoresDgm: any = myData['ValoresAreaDGM'].Content.Items;

      const items: any[] = [];
      let indexBegin: number = 0;
      let indexEnd: number = 0;

      for (const index in elementosDgm) {

        indexEnd = indexBegin + AtributosDgm[index].Content.Items.length - 1;

        items.push({
          element: elementosDgm[index].Name,
          attributes: AtributosDgm[index].Content.Items.map((v: any) => v.Name),
          values: ValoresDgm.filter((element: any, i: number) => {
  ...
  graficoCalibraciones(data: any): void {
    const info: any = this._service.getGraficoDivision(data[2], 1, `REGISTRO CALIBRACIONES`, ' ÚLTIMOS 3 MESES', 'N° Calibraciones', null, true);
    setTimeout(() => {
