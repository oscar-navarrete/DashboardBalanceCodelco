...
@Component({
  selector: 'app-tabla-dgm',
  templateUrl: './tabla-dgm.component.html',
  styleUrls: ['./tabla-dgm.component.scss']
})
export class TablaDGMComponent implements OnInit {
  ...
  datosPatioCatodosDgm: any[] = [];
  datosAreaHumedaDgm: any[] = [];
  datosAreaSecaDgm: any[] = [];
  // datosOxidosDmh: any[] = [];
  valores: any[] = [];
  valor: any = {};
  ...
  generaTablas() {
    this.getDataPatioCatodos('tabladgmpatiocatodos');
    this.getDataAreaHumeda('tabladgmareahumeda');
    this.getDataAreaSeca('tabladgmareaseca');
    this.getFechaPortal();
  }
  ...
  async getDataPatioCatodos(queryCode: string) {
    ...
    await this.ds.getDataFromAPI(body).then((data: any) => {
      this.datosPatioCatodosDgm = this.creaTabla(data, 'ElementosTDGMPatioCatodos', 'ValoresTDGMPatioCatodos', 'AtributosTDGMPatioCatodos');
    });
  }
  ...
  async getDataAreaHumeda(queryCode: string) {
    const body: any = this.getBody(queryCode, 'ds-001');
    await this.ds.getDataFromAPI(body).then((data: any) => {
      this.datosAreaHumedaDgm = this.creaTabla(data, 'ElementosTDGMAreaHumeda', 'ValoresTDGMAreaHumeda', 'AtributosTDGMAreaHumeda');
    });
  }
  ...
  async getDataAreaSeca(queryCode: string) {
    const body: any = this.getBody(queryCode, 'ds-001');
    await this.ds.getDataFromAPI(body).then((data: any) => {
      this.datosAreaSecaDgm = this.creaTabla(data, 'ElementosTDGMAreaSeca', 'ValoresTDGMAreaSeca', 'AtributosTDGMAreaSeca');
    });
  }
