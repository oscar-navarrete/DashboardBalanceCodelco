...
@Component({
  selector: 'app-monitoreobalancedch',
  templateUrl: './monitoreobalancedch.component.html',
  styleUrls: ['./monitoreobalancedch.component.scss']
})
export class MonitoreobalancedchComponent implements OnInit {
  ...
  async getDatosAreas() {
    const body: any = this._service.getBody('DCH', 'ds-001');
    await this.ds.getDataFromAPI(body).then((data: any) => {
      const myData: any = data;

      const elementosDch: any = myData['ElementosAreaDCH'].Content.Items;
      const AtributosDch: any = myData['AttributosAreaDCH'].Content.Items;
      const ValoresDch: any = myData['ValoresAreaDCH'].Content.Items;

      const items: any[] = [];
      let indexBegin: number = 0;
      let indexEnd: number = 0;

      for (const index in elementosDch) {

        indexEnd = indexBegin + AtributosDch[index].Content.Items.length - 1;

        items.push({
          element: elementosDch[index].Name,
          attributes: AtributosDch[index].Content.Items.map((v: any) => v.Name),
          values: ValoresDch.filter((element: any, i: number) => {
  ...
  graficoCalibraciones(data: any): void {
    const info: any = this._service.getGraficoDivision(data[1], 1, `REGISTRO CALIBRACIONES`, ' ÚLTIMOS 3 MESES', 'N° Calibraciones', null, true);
    setTimeout(() => {
