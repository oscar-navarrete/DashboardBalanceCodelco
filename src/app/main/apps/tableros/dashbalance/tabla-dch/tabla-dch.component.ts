...
@Component({
  selector: 'app-tabla-dch',
  templateUrl: './tabla-dch.component.html',
  styleUrls: ['./tabla-dch.component.scss']
})
export class TablaDCHComponent implements OnInit {
  ...
  datosConcentradoraDch: any[] = [];
  datosFundicionDch: any[] = [];
  datosRefineriaDch: any[] = [];
  datosHidroDch: any[] = [];
  // datosOxidosDmh: any[] = [];
  valores: any[] = [];
  valor: any = {};
  ...
  generaTablas() {
    this.getDataConcentradora('tabladchconcentradora');
    this.getDataFundicion('tabladchfundicion');
    this.getDataRefineria('tabladchrefineria');
    this.getDataHidro('tabladchhidro');
    this.getFechaPortal();
  }

  getDataConcentradora(queryCode: string) {
    const body: any = this.getBody(queryCode, 'ds-001');
    this.ds.getDataFromAPI(body).then((data: any) => {
      this.datosConcentradoraDch = this.creaTabla(data, 'ElementosTDCHConcentradora', 'ValoresTDCHConcentradora', 'AtributosTDCHConcentradora');
    });
  }

  getDataFundicion(queryCode: string) {
    const body: any = this.getBody(queryCode, 'ds-001');
    this.ds.getDataFromAPI(body).then((data: any) => {
      this.datosFundicionDch = this.creaTabla(data, 'ElementosTDCHFundicion', 'ValoresTDCHFundicion', 'AtributosTDCHFundicion');
    });
  }

  getDataRefineria(queryCode: string) {
    const body: any = this.getBody(queryCode, 'ds-001');
    this.ds.getDataFromAPI(body).then((data: any) => {
      this.datosRefineriaDch = this.creaTabla(data, 'ElementosTDCHRefineria', 'ValoresTDCHRefineria', 'AtributosTDCHRefineria');
    });
  }

  getDataHidro(queryCode: string) {
    const body: any = this.getBody(queryCode, 'ds-001');
    this.ds.getDataFromAPI(body).then((data: any) => {
      this.datosHidroDch = this.creaTabla(data, 'ElementosTDCHHidro', 'ValoresTDCHHidro', 'AtributosTDCHHidro');
    });
  }
