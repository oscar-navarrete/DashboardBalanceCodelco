...
@Component({
  selector: 'app-monitoreobalancedmh',
  templateUrl: './monitoreobalancedmh.component.html',
  styleUrls: ['./monitoreobalancedmh.component.scss']
})

export class MonitoreobalancedmhComponent implements OnInit {
  ...
  MonitoreoDMH(data: any): void {
    console.log(data[4].Calidad_Dato_Mes_Actual);
    const infoanterior: any = this.getGaugueConfig(data[4], data[4].Calidad_Dato_Mes_Anterior, 1, `MES`, ' ANTERIOR', '');
    const infoactual: any = this.getGaugueConfig(data[4], data[4].Calidad_Dato_Mes_Actual, 2, `ACUMULADO`, ' MES ACTUAL', '');
    setTimeout(() => {
  ...
  async getDatosAreas() {
    const body: any = this._service.getBody('DMH', 'ds-001');
    await this.ds.getDataFromAPI(body).then((data: any) => {
      const myData: any = data;

      const elementosDmh: any = myData['ElementosAreaDMH'].Content.Items;
      const AtributosDmh: any = myData['AttributosAreaDMH'].Content.Items;
      const ValoresDmh: any = myData['ValoresAreaDMH'].Content.Items;

      const items: any[] = [];
      let indexBegin: number = 0;
      let indexEnd: number = 0;

      for (const index in elementosDmh) {

        indexEnd = indexBegin + AtributosDmh[index].Content.Items.length - 1;

        items.push({
          element: elementosDmh[index].Name,
          attributes: AtributosDmh[index].Content.Items.map((v: any) => v.Name),
          values: ValoresDmh.filter((element: any, i: number) => {
  ...
  graficoCalibraciones(data: any): void {
    const info: any = this._service.getGraficoDivision(data[3], 1, `REGISTRO CALIBRACIONES`, ' ÚLTIMOS 3 MESES', 'N° Calibraciones', null, true);
    setTimeout(() => {
