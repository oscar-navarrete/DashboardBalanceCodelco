...
@Component({
  selector: 'app-monitoreobalancedsa',
  templateUrl: './monitoreobalancedsa.component.html',
  styleUrls: ['./monitoreobalancedsa.component.scss']
})
export class MonitoreobalancedsaComponent implements OnInit {
  ...
  MonitoreoDMH(data: any): void {
    const infoanterior: any = this.getGaugueConfig(data[6], data[6].Calidad_Dato_Mes_Anterior, 1, `MES`, ' ANTERIOR', '');
    const infoactual: any = this.getGaugueConfig(data[6], data[6].Calidad_Dato_Mes_Actual, 2, `ACUMULADO`, ' MES ACTUAL', '');
    setTimeout(() => {
  ...
  async getDatosAreas() {
    const body: any = this._service.getBody('DSA', 'ds-001');
    await this.ds.getDataFromAPI(body).then((data: any) => {
      const myData: any = data;

      const elementosDsa: any = myData['ElementosAreaDSA'].Content.Items;
      const AtributosDsa: any = myData['AttributosAreaDSA'].Content.Items;
      const ValoresDsa: any = myData['ValoresAreaDSA'].Content.Items;

      const items: any[] = [];
      let indexBegin: number = 0;
      let indexEnd: number = 0;

      for (const index in elementosDsa) {

        indexEnd = indexBegin + AtributosDsa[index].Content.Items.length - 1;

        items.push({
          element: elementosDsa[index].Name,
          attributes: AtributosDsa[index].Content.Items.map((v: any) => v.Name),
          values: ValoresDsa.filter((element: any, i: number) => {
  ...
  graficoCalibraciones(data: any): void {
    const info: any = this._service.getGraficoDivision(data[5], 1, `REGISTRO CALIBRACIONES`, ' ÚLTIMOS 3 MESES', 'N° Calibraciones', null, true);
    setTimeout(() => {
