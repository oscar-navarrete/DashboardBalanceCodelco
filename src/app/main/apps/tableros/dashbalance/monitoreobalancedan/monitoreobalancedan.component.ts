...
@Component({
  selector: 'app-monitoreobalancedan',
  templateUrl: './monitoreobalancedan.component.html',
  styleUrls: ['./monitoreobalancedan.component.scss']
})

export class MonitoreobalancedanComponent implements OnInit {
  ...
  MonitoreoDMH(data: any): void {
    const infoanterior: any = this.getGaugueConfig(data[1], data[1].Calidad_Dato_Mes_Anterior, 1, `MES`, ' ANTERIOR', '');
    const infoactual: any = this.getGaugueConfig(data[1], data[1].Calidad_Dato_Mes_Actual, 2, `ACUMULADO`, ' MES ACTUAL', '');
    setTimeout(() => {
  ...
  async getDatosAreas() {
    const body: any = this._service.getBody('DAN', 'ds-001');
    await this.ds.getDataFromAPI(body).then((data: any) => {
      const myData: any = data;

      const elementosDan: any = myData['ElementosAreaDAN'].Content.Items;
      const AtributosDan: any = myData['AttributosAreaDAN'].Content.Items;
      const ValoresDan: any = myData['ValoresAreaDAN'].Content.Items;

      const items: any[] = [];
      let indexBegin: number = 0;
      let indexEnd: number = 0;

      for (const index in elementosDan) {

        indexEnd = indexBegin + AtributosDan[index].Content.Items.length - 1;

        items.push({
          element: elementosDan[index].Name,
          attributes: AtributosDan[index].Content.Items.map((v: any) => v.Name),
          values: ValoresDan.filter((element: any, i: number) => {
  ...
  graficoCalibraciones(data: any): void {
    const info: any = this._service.getGraficoDivision(data[0], 1, `REGISTRO CALIBRACIONES`, ' ÚLTIMOS 3 MESES', 'N° Calibraciones', null, true);
    setTimeout(() => {
