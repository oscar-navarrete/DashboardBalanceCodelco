...
@Component({
  selector: 'app-monitoreobalancedve',
  templateUrl: './monitoreobalancedve.component.html',
  styleUrls: ['./monitoreobalancedve.component.scss']
})

export class MonitoreobalancedveComponent implements OnInit {
  ...
  MonitoreoDMH(data: any): void {
    const infoanterior: any = this.getGaugueConfig(data[8], data[8].Calidad_Dato_Mes_Anterior, 1, `MES`, ' ANTERIOR', '');
    const infoactual: any = this.getGaugueConfig(data[8], data[8].Calidad_Dato_Mes_Actual, 2, `ACUMULADO`, ' MES ACTUAL', '');
    setTimeout(() => {
  ...
  async getDatosAreas() {
    const body: any = this._service.getBody('DVE', 'ds-001');
    await this.ds.getDataFromAPI(body).then((data: any) => {
      const myData: any = data;

      const elementosDve: any = myData['ElementosAreaDVE'].Content.Items;
      const AtributosDve: any = myData['AttributosAreaDVE'].Content.Items;
      const ValoresDve: any = myData['ValoresAreaDVE'].Content.Items;

      const items: any[] = [];
      let indexBegin: number = 0;
      let indexEnd: number = 0;

      for (const index in elementosDve) {

        indexEnd = indexBegin + AtributosDve[index].Content.Items.length - 1;

        items.push({
          element: elementosDve[index].Name,
          attributes: AtributosDve[index].Content.Items.map((v: any) => v.Name),
          values: ValoresDve.filter((element: any, i: number) => {
  ...
  graficoCalibraciones(data: any): void {
    const info: any = this._service.getGraficoDivision(data[7], 1, `REGISTRO CALIBRACIONES`, ' ÚLTIMOS 3 MESES', 'N° Calibraciones', null, true);
    setTimeout(() => {
